...
public class EventDAO {
	...
	private static String REMOVEEVENTSQL="DELETE FROM  Event WHERE (startDate= ? and startTime=? )and title=? ";
...
public static void insert(Event evnt) throws Exception {
	...
	public static List<Event> readByDay(java.util.Date date) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				flag = true;
				Event evnt = new Event(rs.getString(1), rs.getString(2), rs.getString(3),rs.getDate(4), rs.getTime(5), rs.getDate(6), rs.getTime(7));
				evntList.add(evnt);
			}
			if (!flag) {
...

{
	...
	try {
		...
		if (rs.next()) {
			flag = true;
  		    evnt = new Event(rs.getString(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getTime(5), rs.getDate(6), rs.getTime(7));
  		    }
...
public static  void removeEvent(java.util.Date d, java.util.Date t,String title) 

{
	...
	try {
		...
		java.sql.Date sStartDate = new java.sql.Date(d.getTime());
		System.out.println(sStartDate);
		pStmt.setDate(1, sStartDate);
//		pStmt.setDate(2, sStartDate);
		java.sql.Time sStartTime = new java.sql.Time(t.getTime());
...
//		System.out.println(evnt.getTitle());
	...
	public static DefaultTableModel readByMonth(java.util.Date date) 
...
//		DefaultTableModel evList = readByWeek(new java.util.Date());
//
//		
//
